...
### Step 1: 实现 `readPreferredNickname()`
...
### Step 3: 增强 Summarizer 昵称识别 Prompt
...
```
Step 1  实现 readPreferredNickname()          ~15min   阻塞其他步骤
  ↓
Step 2  Prompt 注入昵称指引                      ~20min   核心价值交付
  ↓
Step 3  增强 Summarizer 识别 Prompt              ~10min   提升识别覆盖率
  ↓
Step 4  Tool Reply 路径传入昵称                   ~10min   补全覆盖面
  ↓
Step 5  端到端验证                               ~15min   确认功能完整
```

建议一次性完成 Step 1-4，然后统一验证。总改动量约 4 个文件、~60 行新增代码。
...
1. **`assembleFallback` 也读取昵称**：当前 `assembleFallback` 没有调用 `readPreferredNickname`（line 206-233），可顺手补上，确保降级路径也有昵称。

2. **`composeMissingParamsReply` 注入昵称**：当前追问缺失参数的回复没有昵称语境，可在 systemContent 组装中加入昵称指引。
