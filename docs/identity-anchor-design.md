...
- **跨会话长期记住**：若希望「住在浦东」在所有会话中都被当作已知身份，需在**身份锚定**中**手动**添加一条（如「常住地：上海浦东新区」）。身份锚定按设计不随对话自动更新。
- **可选产品增强**：未来可做「身份锚定候选」——当检测到用户明确声明常住地/身份且 worldState 已更新时，由总结或独立流程产出候选条目，前端展示「是否将“常住地：上海浦东新区”加入身份锚定？」用户确认后再写入，既不违背「不自动写入」边界，又减少用户手动维护。
