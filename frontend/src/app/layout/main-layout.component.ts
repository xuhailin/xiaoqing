    ...
    <div class="app-shell">
      <aside class="app-sidebar">
        <div class="app-brand-card">
          <button
            type="button"
            class="app-brand app-brand--compact"
            (click)="selectPrimary('chat')"
            title="小晴"
          >
  ...
  protected readonly workspaceSubNavItems: readonly WorkspaceSubNavItem[] = [
    {
      ...
      label: '工作台',
      icon: 'layoutTemplate',
      description: '围绕事情的生命周期：灵感、事项、推进、完成。',
    },
    {
      value: 'reminder',
      label: '提醒',
      icon: 'bell',
      description: '单次或周期性提醒，随时配置和管理。',
    },
    {
      value: 'plan',
      label: '调度',
      icon: 'calendarCheck',
      description: '管理自动提醒、自动执行规则和周期任务。',
    },
    {
      value: 'execution',
      label: '日志',
  ...
  currentPageHeader(): { title: string; description: string } {
    ...
    if (primary === 'workspace') {
      return this.findPageHeader(
        '工作台',
        this.workspaceSubNavItems,
        this.currentWorkspaceSubnav(),
      );
    }
