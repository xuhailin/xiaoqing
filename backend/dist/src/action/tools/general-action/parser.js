...
exports.parseGeneralAction = parseGeneralAction;
...
function pickUrl(input, kv) {
    if (kv.url)
        return kv.url;
    const m = input.match(/https?:\/\/[^\s]+/i);
    return m?.[0] ?? '';
}
function missing(message, reason) {
    return { status: 'validation_error', message, reason };
}
function parseGeneralAction(input) {
    const raw = String(input ?? '').trim();
    if (!raw)
        return missing('动作输入为空', 'empty_input');
    const lowered = raw.toLowerCase();
    const kv = parseKeyValuePairs(raw);
    if (/^(open|visit|goto)\b/i.test(raw) ||
        /^(打开|访问|前往|浏览)\s*/.test(raw) ||
        lowered.startsWith('browser.goto')) {
        const url = pickUrl(raw, kv);
        if (!url)
            return missing('browser.goto 需要 url', 'missing_url');
    ...
    if (lowered.startsWith('browser.click') || /(点击|click)/i.test(raw)) {
        ...
        if (!url)
            return missing('browser.click 需要 url', 'missing_url');
        if (!selector)
            return missing('browser.click 需要 selector', 'missing_selector');
    ...
    if (lowered.startsWith('browser.fill') || /(输入|填写|fill)/i.test(raw)) {
        ...
        if (!url)
            return missing('browser.fill 需要 url', 'missing_url');
        if (!selector)
            return missing('browser.fill 需要 selector', 'missing_selector');
        if (!value)
            return missing('browser.fill 需要 value', 'missing_value');
    ...
    if (lowered.startsWith('browser.wait') || /(等待|wait)/i.test(raw)) {
        ...
        const selector = kv.selector ?? kv.sel ?? '';
        if (!url)
            return missing('browser.wait 需要 url', 'missing_url');
        if (!selector)
            return missing('browser.wait 需要 selector', 'missing_selector');
    ...
    if (lowered.startsWith('file.read') || /(读取文件|读文件|read file)/i.test(raw)) {
        ...
        if (!target)
            return missing('file.read 需要 path', 'missing_path');
    ...
    if (lowered.startsWith('file.write') || /(写入文件|写文件|write file)/i.test(raw)) {
        ...
        if (!target)
            return missing('file.write 需要 path', 'missing_path');
        if (!content)
            return missing('file.write 需要 content', 'missing_content');
    ...
    if (lowered.startsWith('file.exists') || /(检查文件|文件是否存在|exists)/i.test(raw)) {
        const target = kv.path ?? kv.file ?? '';
        if (!target)
            return missing('file.exists 需要 path', 'missing_path');
