// @ts-nocheck
/**
 * Memory duplicate cleanup (dry-run by default).
 *
 * Examples:
 *   npx ts-node scripts/cleanup-memory-duplicates.ts
 *   npx ts-node scripts/cleanup-memory-duplicates.ts --category=judgment_pattern --apply
 *   npx ts-node scripts/cleanup-memory-duplicates.ts --type=long --threshold=0.74 --apply
 */
...
function categoryCjkWeight(category) {
  ...
  if (category === 'soft_preference') return 0.5;
  return 0.4;
}
