...
function findStep(steps: LocalSkillStepResult[], id: string): LocalSkillStepResult | undefined {
  return steps.find((step) => step.id === id);
}

function getObjectMeta(step: LocalSkillStepResult | undefined): Record<string, unknown> {
  if (!step?.meta || typeof step.meta !== 'object') return {};
  return step.meta;
}

function buildRepoSummary(steps: LocalSkillStepResult[], success: boolean): string {
  ...
  if (readPackageStep?.success && readPackageStep.content) {
    try {
      const pkg = JSON.parse(readPackageStep.content) as Record<string, unknown>;
  ...
  const entriesRaw = Array.isArray(listSrcMeta.entries)
    ? listSrcMeta.entries.filter((item): item is string => typeof item === 'string')
    : [];
  ...
  if (!success) {
    ...
    if (failedStep) {
      lines.push(
        `Failed at step ${failedStep.index} (${failedStep.id}): ${failedStep.error ?? 'unknown error'}`,
      );
    }
  }
...
export const REPO_SUMMARY_SKILL: LocalSkillDefinition = {
  name: 'repo-summary',
