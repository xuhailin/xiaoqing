...
额外：如果用户明确表达了“希望被怎么称呼”的偏好，在 preferredNickname 字段返回该昵称（≤10字）。
可触发表达（仅举例，不限于）：
- 直接声明："叫我XX" / "以后叫我XX" / "称呼我XX" / "我叫XX"
- 修改/纠正："别叫我XX了" / "换个称呼" / "还是叫我XX" / "这个昵称太XX了，叫我YY"
- 确认 AI 提议：用户回复 "好" / "可以" / "就这个" / "就这样"
如果用户只说“别叫我XX了”（或类似拒称）但未给出替代昵称，preferredNickname 必须返回 null（不要清空旧值）。
仅当用户有明确的称呼偏好意图时才提取；不要从姓名/语境自动推断昵称。
