...
import {
  ...
  type DailyMomentScoreBreakdown,
  type DailyMomentSuppressionReason,
  type DailyMomentTriggerContext,
  type DailyMomentTriggerEvaluation,
} from './daily-moment.types';

const clamp01 = (v: number): number => Math.max(0, Math.min(1, v));

@Injectable()
export class DailyMomentTriggerEvaluator {
  private readonly lowThreshold = 0.48;
  private readonly highThreshold = 0.62;

  evaluate(
    messages: DailyMomentChatMessage[],
    context: DailyMomentTriggerContext,
    scoreBias = 0,
  ): DailyMomentTriggerEvaluation {
    ...
    const total = clamp01(
      fun * 0.3 + atmosphere * 0.2 + completeness * 0.2 + companionship * 0.2 + initiative * 0.1,
    );
  ...
  private resolveSuppression(
    messages: DailyMomentChatMessage[],
    context: DailyMomentTriggerContext,
  ): DailyMomentSuppressionReason | null {
    if (context.policyBlocked) return 'policy_blocked';
    if (context.hasRecentTriggerInSession) return 'cooldown_active';
    ...
    if (seriousness === 'focused') return 'serious_or_sensitive_context';

    const emotion = String(context.detectedEmotion ?? '').toLowerCase();
    const latestUser = [...messages].reverse().find((m) => m.role === 'user')?.content ?? '';
    if (
      ['low', 'hurt', 'anxious'].includes(emotion) ||
  ...
  private scoreFun(messages: DailyMomentChatMessage[], reasons: string[]): number {
    ...
    if (markerHits > 0) reasons.push(`fun-marker:${markerHits}`);
    return clamp01(markerHits * 0.22 + turnBounce * 0.45);
  }

  private scoreAtmosphere(messages: DailyMomentChatMessage[], reasons: string[]): number {
    const text = messages.map((m) => m.content).join('\n');
    ...
    if (warm + light > 0) reasons.push(`atmosphere-positive:${warm + light}`);
    return clamp01(0.15 + warm * 0.18 + light * 0.12 - negative * 0.2);
  }

  private scoreCompleteness(messages: DailyMomentChatMessage[], reasons: string[]): number {
    if (messages.length < 3) return 0.15;
    const n = messages.length;
    const lengthScore = clamp01((n - 3) / 7); // 3~10条拉满
    const alternation = this.turnBounce(messages);
    const text = messages.map((m) => m.content).join('\n');
    const pivot = this.countHits(text, [/(结果|本来|后来|但是|然后|反而|那就好|好吧)/g]);
    if (pivot > 0) reasons.push(`snippet-pivot:${pivot}`);
    return clamp01(lengthScore * 0.4 + alternation * 0.4 + Math.min(1, pivot * 0.35) * 0.2);
  }

  private scoreCompanionship(messages: DailyMomentChatMessage[], reasons: string[]): number {
    const text = messages.map((m) => m.content).join('\n');
    const pairMentions = this.countHits(text, [/(你|我|我们|咱们|一起|陪你|陪我)/g]);
    const receiving = this.countHits(text, [/(接住|懂你|在这儿|我在|那就好)/g]);
    if (pairMentions > 0) reasons.push(`companionship-pair:${pairMentions}`);
    return clamp01(pairMentions * 0.06 + receiving * 0.24 + 0.1);
  }

  private scoreInitiative(messages: DailyMomentChatMessage[], reasons: string[]): number {
    const userMessages = messages.filter((m) => m.role === 'user');
    if (userMessages.length === 0) return 0;
    const ratio = userMessages.length / messages.length;
    const avgLen =
      userMessages.reduce((acc, m) => acc + m.content.trim().length, 0) / userMessages.length;
    const continuedTurns = userMessages.length >= 2 ? 1 : 0;
    if (continuedTurns) reasons.push('initiative:continued-user-turns');
    return clamp01(ratio * 0.45 + clamp01(avgLen / 18) * 0.35 + continuedTurns * 0.2);
  }

  private turnBounce(messages: DailyMomentChatMessage[]): number {
    if (messages.length <= 1) return 0;
    let switched = 0;
    for (let i = 1; i < messages.length; i += 1) {
      if (messages[i - 1].role !== messages[i].role) switched += 1;
    }
  ...
  private inferMoodTag(b: DailyMomentScoreBreakdown): DailyMomentMoodTag | undefined {
    if (b.fun >= 0.72) return '被逗了一下';
    if (b.companionship >= 0.7) return '被接住';
    if (b.atmosphere >= 0.68) return '温柔';
    if (b.completeness >= 0.72 && b.fun >= 0.5) return '小反转';
    if (b.total >= 0.62) return '轻松';
    return undefined;
  }
}
