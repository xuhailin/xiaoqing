...
export class ConversationService {
  ...
  constructor(
    ...
    // Feature flags: 默认值见注释
    this.featureImpressionCore = config.get('FEATURE_IMPRESSION_CORE') !== 'false';       // default on
    this.featureImpressionDetail = config.get('FEATURE_IMPRESSION_DETAIL') === 'true';    // default off
    this.featureKeywordPrefilter = config.get('FEATURE_KEYWORD_PREFILTER') !== 'false';   // default on
    this.featureLlmRank = config.get('FEATURE_LLM_RANK') === 'true';                      // default off
    this.featureDynamicTopK = config.get('FEATURE_DYNAMIC_TOPK') !== 'false';             // default on
    this.featureShortSummary = config.get('FEATURE_MEMORY_SHORT_SUMMARY') === 'true';     // default off
    this.featureDebugMeta = config.get('FEATURE_DEBUG_META') === 'true';                   // default off
    this.featureOpenClaw = config.get('FEATURE_OPENCLAW') === 'true';                     // default off
    this.featureAutoSummarize = config.get('FEATURE_AUTO_SUMMARIZE') !== 'false';         // default on
    ...
    this.featureInstantSummarize = config.get('FEATURE_INSTANT_SUMMARIZE') !== 'false'; // default on
  }
  ...
  async delete(conversationId: string): Promise<{
    ...
    // 2. 删除 sourceMessageIds 与该对话有交集的记忆（硬删除）
    ...
    // 3. 删除对话（Message 通过 onDelete: Cascade 自动删除）
  ...
  private async processTurnInternal(context: TurnContext): Promise<SendMessageResult> {
    ...
    // ── 意图识别 + OpenClaw 分流 ──────────────────────────
    let intentState: DialogueIntentState | null = null;
    ...
    if (hasAnyChatCapability) {
      ...
      if (intentState) {
        ...
        const { merged, filledFromWorldState } = await this.worldState.mergeSlots(
          conversationId,
          intentState,
  ...
  private parseLocalSkillCommand(input: string): string | null {
    const matched = String(input ?? '').trim().match(ConversationService.SKILL_COMMAND_RE);
  ...
  private takeValidCoord(value: unknown): string | undefined {
    ...
    return s && ConversationService.COORD_REGEX.test(s) ? s : undefined;
  }
  ...
  private decideToolPolicy(intentState: DialogueIntentState): ToolPolicyDecision {
    ...
    if (intentState.taskIntent !== 'none' && intentState.taskIntent !== 'dev_task') {
      ...
      if (cap) {
        const action = ConversationService.CAPABILITY_TO_ACTION[cap.name];
        if (action) {
  ...
  private async buildToolReplyAndSave(
    ...
    const userProfileText = this.buildInjectedUserProfileText(
      await this.userProfile.getOrCreate(),
  ...
  private async handleChatReply(
    ...
    const coreTokens = this.featureImpressionCore
      ? estimateTokens(userProfile.impressionCore || '')
      : 0;
    const memoryBudget = Math.max(200, this.maxSystemTokens - personaTokens - coreTokens);

    // ── 记忆选取：两阶段管线 or fallback 全量注入 ───────────
    let finalMemories: Array<{ id: string; type: string; content: string }>;
    ...
    if (this.featureKeywordPrefilter) {
      const recallResult = await trace.wrap('memory-recall', '记忆召回', async () => {
        ...
        // 过滤 topic drift（deferred）的 long memory
        ...
        const selected = this.router.selectMemoriesForInjection(
          activeCandidates,
          budget,
          this.memoryContentMaxChars,
          this.featureShortSummary,
        );
  ...
  private async writeIdentityUpdate(
    ...
    for (const [key, value] of entries) {
      const label = ConversationService.IDENTITY_LABEL_MAP[key];
      if (!label) continue;
  ...
  private shouldInstantSummarize(userContent: string): boolean {
    ...
    return ConversationService.INSTANT_SUMMARIZE_RE.test(userContent);
  }
  ...
  async flushSummarize(conversationId: string): Promise<{ flushed: boolean }> {
    ...
    this.summarizer.summarize(conversationId, newMessageIds).catch((err: Error) =>
