...
以下技能具有真实外部副作用，**禁止**纳入 QA 自动化测试（gate / replay）：

| 技能 | 原因 |
|------|------|
| `book-download` | 触发真实下载，消耗外部资源站流量 |
| `checkin` | 产生真实打卡记录，不可撤销 |
| `timesheet` | 提交真实工时到 OA 系统 |

新增有副作用的技能时，需同步更新本列表和 `.cursor/rules/qa-skill-exclusion.mdc`。
